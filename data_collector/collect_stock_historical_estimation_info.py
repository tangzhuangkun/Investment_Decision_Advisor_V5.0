...
import \
    db_mapper.financial_data.stocks_main_estimation_indexes_historical_data_mapper as stocks_main_estimation_indexes_historical_data_mapper
...
class CollectStockHistoricalEstimationInfo:
    ...
    def the_stocks_that_already_in_db(self, exchange_location_mic, latest_collection_date):
        ...
        saved_stock_info_list = all_tracking_stocks_rf_mapper.AllTrackingStocksRfMapper().get_the_stocks_that_already_in_db_and_need_to_be_tracked(
            exchange_location_mic, latest_collection_date)
    ...
    def the_stocks_that_already_in_db_counter(self, exchange_location_mic, latest_collection_date):
        ...
        :return: 统计数， 如 66
        '''
        saved_stocks_info_counter = len(
            self.the_stocks_that_already_in_db(exchange_location_mic, latest_collection_date))
    ...
    def the_stocks_that_not_in_db(self, exchange_location_mic, latest_collection_date):
        ...
        # 统一处理为大写
        exchange_location_mic = exchange_location_mic.upper()
        not_saved_stock_info_list = all_tracking_stocks_rf_mapper.AllTrackingStocksRfMapper().get_the_stocks_that_not_in_db_but_need_to_be_tracked(
            exchange_location_mic, latest_collection_date)
    ...
    def the_stocks_that_not_in_db_counter(self, exchange_location_mic, latest_collection_date):
        ...
        :return: 统计数， 如 1
        '''
        not_saved_stocks_info_counter = len(
            self.the_stocks_that_not_in_db(exchange_location_mic, latest_collection_date))
    ...
    def paged_demanded_stocks(self, exchange_location_mic, page, size):
        ...
        start_row = page * size
        paged_stock_info = all_tracking_stocks_rf_mapper.AllTrackingStocksRfMapper().get_paged_exchange_demanded_stocks(
            exchange_location_mic, start_row, size)
    ...
    def page_counter_by_page_size_per_page(self, exchange_location_mic):
        ...
        stock_codes_total_counter = \
        all_tracking_stocks_rf_mapper.AllTrackingStocksRfMapper().exchagne_all_tracking_stocks_counter(
            exchange_location_mic)["counter"]
        # 如果分页后，有余数（即未满一页的）
    ...
    def tell_exchange_market_and_determine_what_estimations_need_to_get(self, exchange_location_mic):
        ...
        # 如果为 港交所
            ...
            return ["pe_ttm", "pb", "ps_ttm", "pcf_ttm", "dyr", "sp", "tv", "fc_rights", "bc_rights", "lxr_fc_rights",
                    "mc"]
        else:
            # 日志记录失败
        ...
        for i in range(0, len(date_list), 2):
            ...
            if 'error' in content and content.get('error').get('message') == "Illegal token.":
                ...
                msg = '使用无效TOKEN ' + token + ' ' + '执行函数collect_a_period_time_estimation获取股票代码 ' + \
                      stock_code + ' ' + str(stock_info_dict.get(stock_code).get(
                    'stock_name')) + ' ' + colleting_start_date + ' ' + colleting_end_date + ' 失败'
                custom_logger.CustomLogger().log_writter(msg, 'error')
                return self.collect_a_period_time_estimation(stock_code, stock_info_dict, colleting_start_date,
                                                             colleting_end_date, exchange_location_mic)
    ...
    def save_content_into_db(self, content, stock_info_dicts, range):
        ...
        for piece in content["data"]:
            ...
            # 存入数据库
            stocks_main_estimation_indexes_historical_data_mapper.StocksMainEstimationIndexesHistoricalDataMapper().save_stock_info(
                stock_code, stock_name, date, exchange_location, exchange_location_mic, pe_ttm, pe_ttm_nonrecurring, pb,
                pb_wo_gw, ps_ttm, pcf_ttm, ev_ebit, stock_yield, dividend_yield, share_price, turnover, fc_rights,
                bc_rights, lxr_fc_rights, shareholders, market_capitalization, circulation_market_capitalization,
                free_circulation_market_capitalization, free_circulation_market_capitalization_per_capita,
                financing_balance, securities_balances, stock_connect_holding_amount, "理杏仁API")
    ...
    def is_existing(self, stock_code, stock_name, date):
        ...
        existing = stocks_main_estimation_indexes_historical_data_mapper.StocksMainEstimationIndexesHistoricalDataMapper().is_this_stock_existing_info(
    ...
    def latest_collection_date(self, exchange_location_mic):
        ...
        latest_collection_date = stocks_main_estimation_indexes_historical_data_mapper.StocksMainEstimationIndexesHistoricalDataMapper().get_the_latest_date_of_exchange(
            exchange_location_mic)
        # 如果数据库无最新的日期，为空
        if latest_collection_date['p_day'] == None:
            # 返回起始日期，2010-01-01
            return self.estimation_start_date
        return latest_collection_date['p_day']

    def collect_the_lacking_dates_estimation(self, saved_stock_info_dict, latest_collection_date,
                                             exchange_location_mic):
        '''
        收集 特定日期，数据库中暂时缺失的估值
    ...
    def main(self):
        ...
        for exchange_location_mic in all_exchange_locaiton_mics:
            ...
            if (latest_collection_date == self.estimation_start_date):
                ...
                exchange_location_stock_info_list = all_tracking_stocks_rf_mapper.AllTrackingStocksRfMapper().get_exchange_all_stocks(
                    exchange_location_mic)
                ...
                # 某个交易所，需要被跟踪, 但暂时不在数据库中的股票个数
                not_saved_stocks_info_counter = self.the_stocks_that_not_in_db_counter(exchange_location_mic,
                ...
                if not_saved_stocks_info_counter > 0:
                    ...
                    # [{'stock_code': '000858', 'stock_name': '五粮液', 'exchange_location': 'sz', 'exchange_location_mic': 'XSHE'},
                    # {'stock_code': '300146', 'stock_name': '汤臣倍健', 'exchange_location': 'sz', 'exchange_location_mic': 'XSHE'},，，，]
                    not_saved_stock_info_list = self.the_stocks_that_not_in_db(exchange_location_mic,
                                                                               latest_collection_date)
                ...
                # [{'stock_code': '000858', 'stock_name': '五粮液', 'exchange_location': 'sz', 'exchange_location_mic': 'XSHE'},
                # {'stock_code': '300146', 'stock_name': '汤臣倍健', 'exchange_location': 'sz', 'exchange_location_mic': 'XSHE'},，，，]
                saved_stock_info_list = self.the_stocks_that_already_in_db(exchange_location_mic,
                                                                           latest_collection_date)
                ...
                if saved_stocks_info_counter > 0:
                    ...
                    if saved_stocks_info_counter < self.page_size:
                        ...
                        for page_num in range(total_page_num):
                            ...
                            self.collect_the_lacking_dates_estimation(saved_stock_info_dict_piece,
                                                                      latest_collection_date,
                                                                      exchange_location_mic)
